...
bool CJsonObject::AddEmptySubObject(const std::string& strKey)
{
	...
	//可能strKey对象不存在但是通过[]访问过，如果不删除，下次再通过[]访问时，将返回empty对象，同理对于所有的Add操作都应该有这个操作
...
cJSON * CJsonObject::CreateDoubleArrayNoKey(double *numbers, int count)
{
	cJSON* pFocusData = NULL;
	if (m_pJsonData != NULL)
	{
	...
	cJSON* pJsonStruct = cJSON_CreateDoubleArray(numbers, count);
	if (pJsonStruct == NULL)
	{
		m_strErrMsg = std::string("create sub empty array error!");
		return NULL;
...
cJSON * CJsonObject::CreateStringArrayNoKey(const char **strings, int count)
{
	cJSON* pFocusData = NULL;
	if (m_pJsonData != NULL)
	{
	...
	cJSON* pJsonStruct = cJSON_CreateStringArray(strings,count);
	if (pJsonStruct == NULL)
	{
		m_strErrMsg = std::string("create sub empty array error!");
		return NULL;
...
bool CJsonObject::ArrayAddNoKeySubArray(cJSON* jArray)
{
	if (NULL == jArray)
	{
		return false;
	...
	cJSON_AddItemToArray(pFocusData, jArray);
...
bool CJsonObject::GetType(int &type) const
{
	if (m_pJsonData != NULL)
	{
		type = m_pJsonData->type;
		return (true);
	}
	else if (m_pExternJsonDataRef != NULL)
	{
		type = m_pExternJsonDataRef->type;
		return (true);
	}
	return(false);
}
...
bool CJsonObject::GetValue(std::string& strValue) const
{
	if (m_pJsonData != NULL)
	{
		if (m_pJsonData->type == cJSON_String)
		{
			strValue = m_pJsonData->valuestring;
			return (true);
		}
	}
	else if (m_pExternJsonDataRef != NULL)
	{
		if (m_pExternJsonDataRef->type == cJSON_String)
		{
			strValue = m_pExternJsonDataRef->valuestring;
			return (true);
		}
	}
	return(false);
...
bool CJsonObject::AddEx(const std::string& strKey,CJsonObject& oJsonObject)
{
	cJSON* pFocusData = NULL;
	if (m_pJsonData != NULL)
	{
	...
	cJSON* pJsonStruct = oJsonObject.GetItemJsonData();
	if (pJsonStruct == NULL)
	{
		m_strErrMsg = std::string("get json data return null ");
		return(false);
	}
	cJSON_AddItemToObject(pFocusData, strKey.c_str(), pJsonStruct);
	...
	if (!oJsonObject.switch_jsonData_and_ExternJsonDataRef())
		return (false);
	std::map<std::string, CJsonObject*>::iterator iter = m_mapJsonObjectRef.find(strKey);
	if (iter != m_mapJsonObjectRef.end())
	{
...
bool CJsonObject::AddNull(const std::string& strKey)
{
	...
	cJSON_AddNullToObject(pFocusData, strKey.c_str());
	if (cJSON_GetObjectItem(pFocusData, strKey.c_str()) == NULL)
	{
		return(false);
	}
...
bool CJsonObject::Add(const std::string& strKey, long lValue)
{
	...
	cJSON* pJsonStruct = cJSON_CreateInt((uint64)lValue, -1);
	if (pJsonStruct == NULL)
	{
		return(false);
	}
...
bool CJsonObject::Add(const std::string& strKey, unsigned long ulValue)
{
	...
	cJSON* pJsonStruct = cJSON_CreateInt(ulValue, 1);
	if (pJsonStruct == NULL)
	{
		return(false);
	}
...
bool CJsonObject::Remove(int witch)
{
	cJSON* pFocusData = NULL;
	if (m_pJsonData == NULL)
	{
	...
	cJSON *  c = cJSON_DetachItemFromArray(pFocusData, witch);
	std::string strKey = "";
	if (c)
		strKey = c->string;
	cJSON_Delete(c);
...
bool CJsonObject::AddEx(CJsonObject& oJsonObject)
{
	cJSON* pFocusData = NULL;
	if (m_pJsonData != NULL)
	{
...
bool CJsonObject::AddNull()
{
	...
	int iArraySizeBeforeAdd = cJSON_GetArraySize(pFocusData);
	cJSON_AddItemToArray(pFocusData, cJSON_CreateNull());
	int iArraySizeAfterAdd = cJSON_GetArraySize(pFocusData);
	if (iArraySizeAfterAdd == iArraySizeBeforeAdd)
	{
		return(false);
...
cJSON * CJsonObject::GetItemJsonData()
{
	if (m_pJsonData != NULL)
	{
		return m_pJsonData;
...
bool CJsonObject::switch_jsonData_and_ExternJsonDataRef()
{
	if (m_pJsonData && m_pExternJsonDataRef)
		return false;
	if (m_pExternJsonDataRef)
		return false;
	m_pExternJsonDataRef = m_pJsonData;
	m_pJsonData = NULL;
	return true;
}

bool CJsonObject::switch_ExternJsonDataRef_and_jsonData()
{
	if (m_pJsonData && m_pExternJsonDataRef)
		return false;
	if (m_pJsonData)
		return false;
	m_pJsonData = m_pExternJsonDataRef;
	m_pExternJsonDataRef = NULL;
	return true;
}
